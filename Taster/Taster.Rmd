...
1)  your reaction times before and after drinking coke
2)  the mean before and after reaction times for the class
...
To read in our data, we will use `read_csv()` command.

Copy the following line of R code into the read_data chunk.

reaction_time \<- read_csv("reaction_time.csv")
...
`A tibble: 103 x 6`. A tibble is a type of dataframe that stores our
...
filter(student=="Myname", name == "time_before"\|name == "time_after")
...
### Drawing a boxplot of the class results
...
class_mean \<- reaction_time %\>% select(-time_before,-time_after) %\>%
...
Now we are going to use this data to draw the boxplots. Coppy and paste
all the following code into r chunk boxplot
